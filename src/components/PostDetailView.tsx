...
function PostFeedItem({
  ...
  return (
    <article id={`post-${postIndex}`}>
      <div
        className="relative w-full aspect-square bg-gray-100 overflow-hidden touch-pan-y"
...
export default function PostDetailView({
  ...
  useEffect(() => {
    ...
    const target = document.getElementById(`post-${postIndex}`);
    if (!container || !target || hasScrolledToInitial.current) return;
